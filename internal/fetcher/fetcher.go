...
func (f *fetcher) Data(ctx context.Context) ([]byte, error) {
	start := time.Now()
	defer func() { logger.Debug("receiver.FetchData: Time spent", "time", time.Since(start).String()) }()

	data, err := json.Marshal(Request{
		APIKey: f.token,
	})
	if err != nil {
		logger.Error("receiver.FetchData: Error marshaling request", "err", err)
		return nil, err
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, f.url.String(), bytes.NewBuffer(data))
	if err != nil {
		logger.Error("receiver.FetchData: Error creating request", "err", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Error("receiver.FetchData: Error sending request", "err", err)
	}
	defer func() { _ = resp.Body.Close() }()

	if resp.StatusCode != http.StatusOK {
		logger.Error("receiver.FetchData: Invalid status code", "statusCode", resp.StatusCode)
		return nil, &HTTPError{Code: resp.StatusCode}
	}

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		logger.Error("receiver.FetchData: Error reading response body", "err", err)
		return nil, err
	}
